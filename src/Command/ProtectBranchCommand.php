...
class ProtectBranchCommand extends BaseCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            $project = $client->api('projects')->search($input->getOption('repository'));
            if (count($project) != 1) {
                throw new RuntimeException('cant identify project');
            }
            $output->writeln('<info>set the branch ' . $input->getOption('branch') . ' in project ' . $input->getOption('repository') . ' to protected</info>');
            $client->api('repositories')->protectBranch($project[0]['id'], $input->getOption('branch'));
